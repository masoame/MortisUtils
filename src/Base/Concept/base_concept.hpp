...
namespace Mortis::BC
{
	...
	template<typename T>
	struct is_packaged_task_impl : std::false_type {};

	template<typename R>
	struct is_packaged_task_impl<std::packaged_task<R()>> : std::true_type {};

	template<typename T>
	inline constexpr bool is_packaged_task_v = is_packaged_task_impl<std::remove_cvref_t<T>>::value;

	template<typename T>
	concept PackagedTask = is_packaged_task_v<T>;
};